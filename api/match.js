...
### 低分精写策略（overallScore < 60 时强制执行）

当诊断总分低于 60 时，说明简历与 JD 匹度很低。必须执行以下差异化精写策略：

**第一步：经历相关性分级**
对简历中每段经历，判断与 JD 的相关程度：
- A 级（强相关）：经历内容与 JD 核心要求直接匹配（同岗位/同技术栈/同业务领域）
- B 级（弱相关）：经历中有部分技能或成果可迁移至 JD 岗位
- C 级（不相关）：经历与 JD 岗位几乎无交集

**第二步：差异化处理**
- A 级经历（精写）：
  • 使用 STAR 框架完整重写，扩展至 4-6 个要点
  • 大量植入 JD 高频关键词（岗位名称、技术工具、业务术语）
  • 补充量化数据（即使需标注【推测范围】）
  • 每个要点必须体现与 JD 的直接关联
  • 在 display.experienceShowcase 中置顶展示

- B 级经历（适写）：
  • 保留 2-3 个要点，选择与 JD 最相关的部分展开
  • 适当植入 JD 关键词，突出可迁移技能
  • 在 display.experienceShowcase 中居中展示

- C 级经历（弱写）：
  • 压缩至 1-2 行概括性描述
  • 删除与 JD 完全无关的技术细节
  • 仅保留能体现通用能力（团队协作、项目管理、问题解决）的要点
  • 在 display.experienceShowcase 中末尾展示或合并为一段
  • 在 optimizations 中将无关细节的 new_text 设为精简版本

**第三步：在 display 中体现分级**
- experienceShowcase 按 A→B→C 级排序
- A 级经历的 highlightedSkills 必须与 JD 高度重合
- 在 matchSummary 中明确指出"以下经历经过差异化精写以突出与目标岗位的关联度"
